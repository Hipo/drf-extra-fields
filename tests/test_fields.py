...
from drf_extra_fields.fields import (Base64ImageField, Base64FileField, DateRangeField, DateTimeRangeField,
                                     FloatRangeField, HybridImageField, IntegerRangeField, LowercaseEmailField,
                                     DecimalRangeField, )
...
class Base64ImageSerializerTests(TestCase):
	...
	def test_validation_error_with_non_file(self):
		...
		"""
		now = datetime.datetime.now()
		serializer = UploadedBase64ImageSerializer(data={
			'created': now, 'file': 'abc'
			})
		self.assertFalse(serializer.is_valid())
		self.assertEqual(serializer.errors, {'file': [Base64ImageField.INVALID_FILE_MESSAGE]})
	...
	def test_not_found(self):
		...
		serializer = DownloadableBase64ImageSerializer(image)
		callable = lambda: serializer.data
		
		self.assertRaises(FileNotFoundError, callable)
...
class PointSerializerTest(TestCase):
	...
	def test_empty_latitude(self):
		now = datetime.datetime.now()
		point = {
			"latitude": 49.8782482189424, "longitude": ""
			}
		serializer = PointSerializer(data={'created': now, 'point': point})
		self.assertFalse(serializer.is_valid())
	...
	def test_invalid_latitude(self):
		now = datetime.datetime.now()
		point = {
			"latitude": 49.8782482189424, "longitude": "fdff"
			}
		serializer = PointSerializer(data={'created': now, 'point': point})
		self.assertFalse(serializer.is_valid())
...
class TestIntegerRangeField(FieldValues):
	...
	valid_inputs = [
		({'lower': '1', 'upper': 2, 'bounds': '[)'}, NumericRange(**{'lower': 1, 'upper': 2, 'bounds': '[)'})),
		({'lower': 1, 'upper': 2}, NumericRange(**{'lower': 1, 'upper': 2})),
		({'lower': 1}, NumericRange(**{'lower': 1})), ({'upper': 1}, NumericRange(**{'upper': 1})),
		({'empty': True}, NumericRange(**{'empty': True})), ({}, NumericRange()), ]
	invalid_inputs = [({'lower': 'a'}, ['A valid integer is required.']),
	...
	outputs = [(NumericRange(**{'lower': '1', 'upper': '2'}), {'lower': 1, 'upper': 2, 'bounds': '[)'}),
	           (NumericRange(**{'empty': True}), {'empty': True}),
	           (NumericRange(), {'bounds': '[)', 'lower': None, 'upper': None}),
	           ({'lower': '1', 'upper': 2, 'bounds': '[)'}, {'lower': 1, 'upper': 2, 'bounds': '[)'}),
	           ({'lower': 1, 'upper': 2}, {'lower': 1, 'upper': 2, 'bounds': None}),
	           ({'lower': 1}, {'lower': 1, 'upper': None, 'bounds': None}),
	           ({'upper': 1}, {'lower': None, 'upper': 1, 'bounds': None}), ({}, {}), ]
	...
	def test_no_source_on_child(self):
		...
		assert str(exc_info.value) == ("The `source` argument is not meaningful when applied to a `child=` field. "
...
class TestIntegerRangeChildAllowNullField(FieldValues):
	...
	valid_inputs = [
		...
		({'lower': 1}, NumericRange(**{'lower': 1})), ({'upper': 1}, NumericRange(**{'upper': 1})),
		({'empty': True}, NumericRange(**{'empty': True})), ({}, NumericRange()),
		({'lower': 1, 'upper': None, 'bounds': '[)'}, NumericRange(**{'lower': 1, 'upper': None, 'bounds': '[)'})),
		({'lower': None, 'upper': 1, 'bounds': '[)'}, NumericRange(**{'lower': None, 'upper': 1, 'bounds': '[)'})), ]
	invalid_inputs = [({'lower': 'a'}, ['A valid integer is required.']),
	                  ('not a dict', ['Expected a dictionary of items but got type "str".']),
	                  ({'foo': 'bar'}, ['Extra content not allowed "foo".']),
	                  ({'lower': 2, 'upper': 1}, ['The start of the range must not exceed the end of the range.']), ]
...
class TestDecimalRangeField(FieldValues):
	...
	valid_inputs = [
		...
		({'lower': 1}, NumericRange(**{'lower': 1})), ({'upper': 1}, NumericRange(**{'upper': 1})),
		({'empty': True}, NumericRange(**{'empty': True})), ({}, NumericRange()), ]
	invalid_inputs = [({'lower': 'a'}, ['A valid number is required.']),
	                  ('not a dict', ['Expected a dictionary of items but got type "str".']),
	                  ({'lower': 2., 'upper': 1.}, ['The start of the range must not exceed the end of the range.']), ]
	outputs = [(NumericRange(**{'lower': '1.1', 'upper': '2'}), {'lower': '1.1', 'upper': '2', 'bounds': '[)'}),
	           (NumericRange(**{'empty': True}), {'empty': True}),
	           (NumericRange(), {'bounds': '[)', 'lower': None, 'upper': None}), (
	           {'lower': Decimal('1.1'), 'upper': "2.3", 'bounds': '[)'},
	           {'lower': "1.1", 'upper': "2.3", 'bounds': '[)'}),
	           ({'lower': Decimal('1.1'), 'upper': "2.3"}, {'lower': "1.1", 'upper': "2.3", 'bounds': None}),
	           ({'lower': 1}, {'lower': "1", 'upper': None, 'bounds': None}),
	           ({'upper': 1}, {'lower': None, 'upper': "1", 'bounds': None}), ({}, {}), ]
...
class TestDecimalRangeFieldWithChildAttribute(FieldValues):
	...
	invalid_inputs = [({'lower': 'a'}, ['A valid number is required.']),
	                  ...
	                  ('not a dict', ['Expected a dictionary of items but got type "str".']),
	                  ({'lower': 2., 'upper': 1.}, ['The start of the range must not exceed the end of the range.']), ]
	outputs = [(NumericRange(**{'lower': '1.1', 'upper': '2'}), {'lower': '1.10', 'upper': '2.00', 'bounds': '[)'}),
	           (NumericRange(**{'empty': True}), {'empty': True}),
	           (NumericRange(), {'bounds': '[)', 'lower': None, 'upper': None}), (
	           {'lower': Decimal('1.1'), 'upper': "2.3", 'bounds': '[)'},
	           {'lower': "1.10", 'upper': "2.30", 'bounds': '[)'}),
	           ({'lower': Decimal('1.1'), 'upper': "2.3"}, {'lower': "1.10", 'upper': "2.30", 'bounds': None}),
	           ({'lower': 1}, {'lower': "1.00", 'upper': None, 'bounds': None}),
	           ({'upper': 1}, {'lower': None, 'upper': "1.00", 'bounds': None}), ({}, {}), ]
...
class TestDateTimeRangeField(TestCase, FieldValues):
	...
	valid_inputs = [({
		                 'lower': '2001-01-01T13:00:00Z', 'upper': '2001-02-02T13:00:00Z', 'bounds': '[)'
		                 }, DateTimeTZRange(**{
		...
		'upper': datetime.datetime(2001, 2, 2, 13, 00, tzinfo=pytz.utc), 'bounds': '[)'
		})), ({
			      'lower': '2001-01-01T13:00:00Z', 'bounds': '[)'
			      }, DateTimeTZRange(**{
		'lower': datetime.datetime(2001, 1, 1, 13, 00, tzinfo=pytz.utc), 'bounds': '[)'
		})), ({'empty': True}, DateTimeTZRange(**{'empty': True})), ({}, DateTimeTZRange()), ]
	invalid_inputs = [({'lower': 'a'}, ['Datetime has wrong format. Use one of these'
	                                    ' formats instead: '
	                  ...
	                  ('not a dict', ['Expected a dictionary of items but got type "str".']), ({
		                                                                                           ...
		                                                                                           'the '
		                                                                                           'range.']), ]
	outputs = [(DateTimeTZRange(**{
		'lower': datetime.datetime(2001, 1, 1, 13, 00, tzinfo=pytz.utc),
		'upper': datetime.datetime(2001, 2, 2, 13, 00, tzinfo=pytz.utc)
		}), {
		            'lower': '2001-01-01T13:00:00Z', 'upper': '2001-02-02T13:00:00Z', 'bounds': '[)'
		            }), (DateTimeTZRange(**{'empty': True}), {'empty': True}),
	           (DateTimeTZRange(), {'bounds': '[)', 'lower': None, 'upper': None}), ({
		                                                                                 'lower' :
			                                                                                 '2001-01-01T13:00:00Z',
		                                                                                 'upper' :
			                                                                                 '2001-02-02T13:00:00Z',
...
class TestDateRangeField(FieldValues):
	...
	valid_inputs = [({
		                 'lower': '2001-01-01', 'upper': '2001-02-02', 'bounds': '[)'
		                 }, DateRange(**{
		'lower': datetime.date(2001, 1, 1), 'upper': datetime.date(2001, 2, 2), 'bounds': '[)'
		})), ({
			      'upper': '2001-02-02', 'bounds': '[)'
			      }, DateRange(**{
		'upper': datetime.date(2001, 2, 2), 'bounds': '[)'
		})), ({
			      'lower': '2001-01-01', 'bounds': '[)'
			      }, DateRange(**{
		'lower': datetime.date(2001, 1, 1), 'bounds': '[)'
		})), ({'empty': True}, DateRange(**{'empty': True})), ({}, DateRange()), ]
	invalid_inputs = [({'lower': 'a'}, ['Date has wrong format. Use one of these'
	                                    ' formats instead: '
	                                    'YYYY-MM-DD.']),
	                  ('not a dict', ['Expected a dictionary of items but got type "str".']), ({
		                                                                                           'lower':
		                                                                                           ...
		                                                                                           'range.']), ]
	outputs = [(DateRange(**{
		'lower': datetime.date(2001, 1, 1), 'upper': datetime.date(2001, 2, 2)
		}), {
		            'lower': '2001-01-01', 'upper': '2001-02-02', 'bounds': '[)'
		            }), (DateRange(**{'empty': True}), {'empty': True}),
...
2001,
	           ...
	           ({'upper': '2001-02-02', 'bounds': '[)'}, {'lower': None, 'upper': '2001-02-02', 'bounds': '[)'}),
	           ({'lower': '2001-01-01', 'bounds': '[)'}, {'lower': '2001-01-01', 'upper': None, 'bounds': '[)'}),
	           ({}, {}), ]
